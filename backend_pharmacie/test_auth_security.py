...
def test_unauthed_invalid_bearer():
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/auth/me", headers={"Authorization": "Bearer invalid_token_12345"}
        )
...
def test_login_endpoint():
    ...
    try:
        ...
        response = requests.post(
            ...
            json={"email": "admin@pharmacie.local", "password": "secure_password_123"},
        )
        ...
        if response.status_code == 200:
            data = response.json()
            print(
                f"   Got tokens: access_token={data.get('access_token')[:20]}..., refresh_token={data.get('refresh_token')[:20]}..."
            )
...
def test_authed_with_cookie(token: str):
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/auth/me", cookies={"access_token": token})
...
def test_admin_upload_unauthed():
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/admin/upload", files={"fichier": ("test.csv", "data")}
        )
...
def run_all_tests():
    ...
    if token:
        ...
        print(
            f"{Colors.WARNING}⚠️  NOTICE: Could not obtain valid token. See 'test_fix_and_create_user.py' to create test user.{Colors.ENDC}"
        )
    ...
    if unauthenticated_blocked:
        print(
            f"{Colors.OKGREEN}{Colors.BOLD}✅ SECURITY FIXED: All unauthenticated requests are properly blocked!{Colors.ENDC}"
        )
    else:
        print(
            f"{Colors.FAIL}{Colors.BOLD}❌ SECURITY VULNERABILITY: Some unauthenticated requests are NOT blocked!{Colors.ENDC}"
        )
