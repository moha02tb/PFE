...
export default function RTLTestComponent() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.status}>Current Language: {language}</Text>
      <Text style={styles.status}>RTL Mode: {isRTL ? 'Active' : 'Inactive'}</Text>
      ...
      <View style={styles.cardTest}>
        ...
        <Text style={styles.cardText}>
          This text should align to the right in RTL mode and to the left in LTR mode. النص العربي
          يجب أن يظهر من اليمين إلى اليسار.
        </Text>
      </View>
    </View>
  );
...
const getStyles = (isDarkMode, isRTL = false) =>
  StyleSheet.create({
    ...
    cardTest: {
      ...
      marginTop: 12,
      ...(isRTL
        ? { borderRightWidth: 4, borderRightColor: '#4CAF50' }
