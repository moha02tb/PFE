...
\begin{longtable}{L{4cm}L{10cm}}
    ...
    BM25 (Okapi) & Algorithme de récupération lexicale pour l'assistant de premiers secours. \\
    Sentence Transformers & Modèle d'embeddings sémantiques (\texttt{all-MiniLM-L6-v2}) pour la récupération dense. \\
...
\begin{tabular}{r@{\quad}l}
  ...
  \small\textbf{Base de données} &
    ...
    \tbadge{BrickRed}{Jest} \\
\end{tabular}
\caption{Stack technologique de PharmacieConnect}
...
L'assistant conversationnel constitue une valeur ajoutée importante de la plateforme PharmacieConnect. Il
fournit des réponses rapides, fiables et sûres aux questions de premiers secours, complétant harmonieusement
la recherche de pharmacies de garde. L'assistant repose sur une architecture de récupération d'informations
hybride (lexicale et sémantique) plutôt que sur la génération, afin de garantir des réponses basées sur des
preuves et d'éliminer les risques d'hallucination.
...
\begin{enumerate}
    ...
    \item \textbf{Récupération parallèle} : Deux algorithmes complémentaires :
        \begin{itemize}
            \item \textbf{BM25 (Okapi)} : Récupération lexicale basée sur fréquence des termes ;
            \item \textbf{Embeddings sémantiques} : Modèle \texttt{all-MiniLM-L6-v2} pour capturer le sens.
        \end{itemize}
    \item \textbf{Fusion hybride} : Normalisation et fusion des scores :
        ...
        \end{itemize}
    \item \textbf{Reclassement et extraction} : Reclassement par similarité, alignement thématique et couverture. Réponse extractive composée du texte récupéré.
    \item \textbf{Contrôle de confiance} : Calcul multi-signaux (score, similarité, cohérence). Fallback sur confiance insuffisante.
...
Le jeu de données initial repose sur FirstAidQA, enrichi et nettoyé pour améliorer la qualité. Les processus
de qualification et d'augmentation sont décrits ci-après.

\begin{table}[H]
    \centering
    \caption{Statistiques du jeu de données de l'assistant}
...
\begin{table}[H]
    \centering
    \caption{Performances de l'assistant après entraînement}
...
\begin{itemize}
    \item Récupération robuste au niveau document (Recall@5 = 94.38\%) ;
    \item Classement fiable (MRR = 0.8627), bons documents en haut de la liste ;
    \item Extraction de spans un point faible (F1 = 0.2990), oracle top-5 F1 = 0.3904 ;
    \item Limitation due à l'appariement réponse-document, non à couverture de récupération ;
    \item Fallback mesuré (2.69\%) confirme bon fonctionnement du mécanisme de confiance.
\end{itemize}
...
intégrer un modèle de question-réponse extractif (de type BERT fine-tuné sur SQuAD) en remplacement de
...
L'assistant bénéficie de deux suites de tests : (1) Smoke Tests avec 14 scénarios pour urgences courantes (étouffement, choc, perte de conscience), tous passant à 100\% ; (2) Tests unitaires avec 3 suites couvrant normalisation, fusion hybride et confiance calculée. Le système fonctionne de manière fiable et aucune hallucination ne peut survenir (extraction basée sur texte récupéré).
...
\begin{figure}[H]
    ...
    \caption{Écran de connexion du tableau de bord administrateur}
...
\begin{figure}[H]
    ...
    \caption{Vue du tableau de bord administrateur}
...
\begin{figure}[H]
    ...
    \caption{Vue du registre des pharmacies}
...
\begin{figure}[H]
    ...
    \caption{Vue de gestion des pharmacies et des gardes}
...
\begin{figure}[H]
    ...
    \caption{Vue du calendrier des gardes}
...
\begin{figure}[H]
    ...
    \caption{Vue cartographique du réseau de pharmacies}
...
\begin{figure}[H]
    ...
    \caption{Interface d'import des pharmacies}
...
\begin{figure}[H]
    ...
    \caption{Interface d'import du planning de garde}
...
\begin{figure}[H]
    ...
    \caption{Vue des journaux d'audit}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.32\textwidth}
        ...
        \captionof{figure}{Calendrier mobile des gardes}
    ...
    \begin{minipage}{0.32\textwidth}
        ...
        \captionof{figure}{Catalogue mobile des médicaments}
    ...
    \begin{minipage}{0.32\textwidth}
        ...
        \captionof{figure}{Assistant mobile de premiers secours}
