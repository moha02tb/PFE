...
PharmacieConnect ne prend tout son sens qu'une fois replacé dans le contexte opérationnel qui a justifié
sa création. La réflexion démarre ici par le fonctionnement concret des gardes pharmaceutiques, se prolonge
par la présentation du cadre du stage externe, puis s'attarde sur les obstacles rencontrés dès qu'une
information de garde doit être recherchée, rectifiée ou diffusée sans délai. La réponse retenue et
l'organisation adoptée pour le projet referment cette mise en contexte.

\section{Présentation du domaine pharmaceutique}

Dans ce projet, le domaine pharmaceutique a surtout été abordé à travers une question très pratique :
comment retrouver une officine disponible lorsque les horaires habituels ne suffisent plus. Les pharmacies
de garde participent à cette continuité de service \cite{cnopt_tableau_garde,cnopt_cropt_gardes}, mais leur
utilité dépend fortement de la précision des informations publiées. Pour PharmacieConnect, cela ramène le
travail à des données concrètes à maîtriser : nom de la pharmacie, adresse, téléphone, gouvernorat, position
GPS, date, horaire et type de garde.
...
d'administration pour la mise à jour des données, tel que synthétisé dans le Tableau~\ref{tab:cadre-institutionnel}.

\begin{table}[H]
    \centering
    \caption{Cadre institutionnel du projet\\ \small Source : Auteur}
    \label{tab:cadre-institutionnel}
    \begin{tabular}{L{4.5cm}L{8.5cm}}
        ...
        Périmètre technique & API backend, interface web d'administration, application mobile et base de données. \\
...
Cet examen des solutions existantes a été guidé par les besoins propres au projet. L'enjeu ne se limite pas
à savoir si un outil affiche une pharmacie : il s'agit de jauger sa capacité à satisfaire plusieurs critères
-- disponibilité des gardes, usage mobile, géolocalisation, fraîcheur des données, processus de mise à jour,
administration, authentification, import de données, auditabilité et couverture du territoire tunisien.
...
Les sites spécialisés et les applications mobiles procurent, eux, une expérience mieux adaptée à la
consultation. Leur efficacité reste toutefois tributaire de la qualité de la base de données, du rythme des
mises à jour et de l'existence d'un espace d'administration apte à contrôler les imports, les droits et les
corrections. Cette nuance compte pour PharmacieConnect, dont l'ambition est de couvrir à la fois la
consultation citoyenne et la gestion opérationnelle des données.

\section{Analyse du marché et solutions existantes}

Pour situer PharmacieConnect, la comparaison n'a pas porté uniquement sur l'existence d'une carte ou d'une
liste de pharmacies. Nous avons surtout regardé ce que chaque solution permet réellement de faire lorsqu'une
donnée doit être cherchée, corrigée ou contrôlée. Les sources institutionnelles, comme le Conseil National
de l'Ordre des Pharmaciens de Tunisie, donnent un cadre fiable pour l'organisation de la profession
\cite{cnopt}. Les annuaires médicaux, dont Med.tn, rendent certaines informations plus accessibles au public
\cite{medtn}. Les services cartographiques, tels que Google Maps et OpenStreetMap, aident à localiser une
adresse, mais ne traitent pas la planification des gardes ni la validation des fichiers importés
\cite{googlemaps,openstreetmap}. L'application Apothicare montre enfin l'intérêt d'un accès mobile aux
pharmacies ouvertes ou de garde \cite{apothicare}, même si l'enjeu de gestion interne reste à analyser selon
...
vérifiée avant la version finale \cite{cnopt,medtn,googlemaps,openstreetmap,apothicare}. Le
Tableau~\ref{tab:comparaison-solutions} en présente la synthèse.
...
Ce panorama révèle que le risque principal ne tient pas seulement à l'affichage des pharmacies, mais aussi
à la maîtrise de la donnée. Une solution peut autoriser la consultation tout en restant limitée si elle
n'explicite pas la manière dont les plannings sont importés, corrigés, validés et tracés. Côté utilisateur
mobile, cette lacune se traduit par une recherche moins fiable ; côté administrateur, par la difficulté de
savoir qui a modifié une donnée, quand et depuis quelle source.
...
\textbf{PharmacieConnect} a donc été pensée comme une réponse progressive aux limites observées. Plutôt que
de séparer la consultation et la gestion, le projet regroupe les deux dans une même chaîne technique, avec un
assistant de premiers secours ajouté au parcours mobile :
...
\begin{itemize}
    ...
    \item un espace d'administration React/Vite qui met à la disposition des administrateurs un environnement
    de gestion pour les pharmacies, les gardes, les médicaments, les imports \gls{csv}, les assistants
    régionaux, les indicateurs et les journaux d'audit ;
...
Cette organisation répond surtout à deux difficultés rencontrées dans l'existant : l'information est parfois
accessible, mais sa mise à jour et son contrôle restent peu visibles. Dans notre solution, le backend garde
la maîtrise des données et des droits, tandis que les interfaces se concentrent chacune sur son usage :
consultation rapide côté citoyen, correction et suivi côté administration. L'assistant ajoute une aide
complémentaire, volontairement prudente, sans prétendre remplacer un professionnel de santé ni les services
d'urgence.
...
rôles, pages d'administration, écrans mobiles et tests de validation. Le Tableau~\ref{tab:sprints-scrum}
présente cette organisation en six sprints.
...
\texttt{backend/}, \texttt{admin/}, \texttt{mobile/} et \texttt{chatbot/}. Le Tableau~\ref{tab:outils-gestion}
récapitule ces deux outils et leurs usages.
...
Ce premier volet a posé le contexte du projet, cerné le domaine pharmaceutique et mis en évidence les
limites des solutions actuelles. Il éclaire aussi le choix d'articuler PharmacieConnect autour d'une
séparation entre backend, administration web et application mobile. La suite du document précise les
besoins qui découlent de ce cadrage.
