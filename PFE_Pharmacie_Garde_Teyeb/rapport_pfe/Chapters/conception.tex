...
Le choix de FastAPI vient principalement du rôle confié au backend dans PharmacieConnect : exposer des
endpoints REST, valider les échanges et garder les règles sensibles côté serveur \cite{fastapi}. Nous avons
séparé les routes HTTP, les services métier, les modèles de données et les schémas de validation afin que les
imports CSV, les gardes, les médicaments, les restrictions régionales et l'audit puissent évoluer sans
entraîner une réécriture directe des écrans web ou mobiles.
...
d'importer des fichiers, de vérifier des indicateurs et de suivre les actions sensibles.
...
consulter un médicament, gérer son compte ou accéder à l'assistant de premiers secours.
...
médicaments, les sessions, les tentatives de connexion, les recherches et l'audit.
...
Dans PharmacieConnect, la sécurité a été placée au niveau du backend, car une règle appliquée uniquement
dans l'interface pourrait être contournée par un appel direct à l'API. Les mécanismes retenus s'inspirent des
recommandations OWASP pour la protection des API~\cite{owasp} et couvrent les étapes sensibles d'une
requête : authentification, autorisation, validation des données, journalisation et configuration du
déploiement. Ce choix protège particulièrement les routes d'administration, les imports CSV, les restrictions
